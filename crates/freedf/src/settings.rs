...
impl MacroKey {
    ...
    /// rdev 키 → 매크로 키 (전역 데스크탑 리스너용). 지원 안 하는 키는 None.
...
pub struct GlobalState {
    ...
    pub refresh_hz: u32,
    /// 커서(펜 닙) 크기 배율 (0.5..2.0, 기본 1.0) — 볼펜/만년필 금속
    /// 닙 커서의 길이·두께에 곱해집니다.
...
impl SessionState {
    ...
    pub fn sanitized(mut self) -> Self {
        ...
        // 커서 크기 배율은 0.5..2.0으로 제한 (NaN 방지 포함).
        self.global.cursor_scale = self.global.cursor_scale.clamp(0.5, 2.0);
