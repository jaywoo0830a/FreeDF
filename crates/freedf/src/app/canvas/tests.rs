...
fn canvas_mesh_to_egui_applies_pan_and_zoom() {
    ...
    let out = canvas_mesh_to_egui(&mesh, egui::pos2(5.0, 6.0), 2.0, 100.0, 200.0);
    assert_eq!(out.vertices.len(), 1);
    let v = out.vertices[0];
    assert!(
        (v.pos.x - (5.0 + 10.0 * 2.0 + 100.0)).abs() < 1e-4,
        "x: {}",
        v.pos.x
    );
    assert!(
        (v.pos.y - (6.0 + 20.0 * 2.0 + 200.0)).abs() < 1e-4,
        "y: {}",
        v.pos.y
    );
    mesh.indices = vec![0];
...
fn clamp_azimuth_hand_keeps_half_plane() {
    ...
    assert!(
        clamp_azimuth_hand(2.2, false).cos() >= 0.0,
        "왼쪽 아래 → 오른쪽"
    );
    assert!(
        clamp_azimuth_hand(-2.2, false).cos() >= 0.0,
