//! 문서별 GUI 세션 상태 — 열었던 페이지, 도구, 펜 설정, 줌/팬 등을
//! 노트 폴더(또는 PDF 옆 사이드카)의 `session.json`에 저장/복원합니다.
//!
//! 앱 전역 기본값(settings.json)과 달리 **문서마다** 독립적으로
//! 마지막 작업 상태를 기억합니다.
...
/// 복원할 GUI 상태. `#[serde(default)]`라 과거 버전 파일(필드 누락)도 안전합니다.
